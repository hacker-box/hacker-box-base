...
function checkPortAndRun(newPort, callback) {
  detect(newPort).then(port => {
    if (port === newPort) {
      callback(port);
      return;
    }
    ...
    console.log("\n\nChanging to port " + nextPort);

    checkPortAndRun(nextPort, callback);
  });
}

// Watch app server files
function watchAppServer(protocol) {
  webpack(wpConfig.server).watch({}, err => {
    if (err) {
      throw err;
    }
    console.log("");
...
function setupCompiler(port, protocol) {
  ...
  compiler.plugin("done", function(stats) {
    ...
    if (!hasErrors && !hasWarnings) {
      ...
      console.log();
      console.log(chalk.cyan("The dev server is running at: " + port));
      console.log();
...
function runDevServer(port, protocol) {
  var devServer = new WebpackDevServer(compiler, {
    ...
    // Allow CORS access, to load from different port.
...
function run(port) {
  ...
  // Update client publicPath, since port might have changed.
  const publicPath = "http://localhost:" + port + "/";
  wpConfig.client.entry.main[0] = wpConfig.client.entry.main[0].replace(
    /\?.*$/,
    "?" + publicPath
  );
  wpConfig.client.output.publicPath = publicPath;

  setupCompiler(port, protocol);
  runDevServer(port, protocol);
}

checkPortAndRun(DEV_SERVER_PORT, run);
